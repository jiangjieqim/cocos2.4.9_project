...
function getCellFiledLen(type){
    switch(type){
        ...
        case TypeDef.double://double
            return 4;
        case TypeDef.ulong://ulong
        case TypeDef.long://long
            return 8;
        case TypeDef.array://array
        case TypeDef.arraystring://arraystring
        case TypeDef.ints://ints
            return 2;
        case TypeDef.DEFAULT_STRING:
        case TypeDef.DEFAULT_NUMBER:
            return 0;
        default:
            throw "getCellFiledLen-->type:"+type + " not exist!";
    }
    return;
}
function objDispose(obj){
    (obj.bs as Byte).clear();
...
function parse(bs) {
    ...
    let tableName = bs.readUTFString();
    obj.tabelName = tableName;
    // console.log('parse tabelName ['+tabelName+'] len ' + bs.length,'strpos',strpos,"dataPos",dataPos);

    // console.log(tabelName);
    let filedLen = bs.readUint16();//bs.readByte();

    // console.log("filedLen", filedLen);
    let fieldList = [];//字段名
    let typeList = [];//类型数组
    if(filedLen < 0){
        let err = `table[${tableName}] 's filedLen(${filedLen}) is bad value!`;
        console.error(err);
        return;
    }
...
function saveArr(s1,type,key){
    let arr = s1.split(key);
    if(s1.length == 0){
        return [];
    }else{
        let nums = [];
        ...
        case TypeDef.arraystring:
            {
                ...
                return saveArr(obj.strlist[index1],type,",");
            }
...
export class ByteCfg{
    ...
    private initDataMap():void{
        // if(!this.indexKEY){
        //     let s = `${this.tableName} indexKEY maybe is (f_id)`
        //     throw s;
        // }
        // this._mDataMap = {};
        ...
        for(let i = 0;i < this.list1.length;i++){
            ...
            this._mDataMap[node[this.indexKEY]] = node;//待优化,读取得时候才解析
        }
        */
        this.b.clear();
    }
